...
disp("finished estimating Q");
%%
subplot(1,2,1);
plot(1:99,squeeze(Q_MB_vals(1,1,1,1,1,:)),'r-'); hold on;
plot(1:99,squeeze(Q_MB_vals(1,1,1,1,2,:)),'b-');
title("model-based");

subplot(1,2,2);
plot(1:99,squeeze(Q_MF_vals(1,1,1,1,1,1,:)),'r.');hold on;
plot(1:99,squeeze(Q_MF_vals(1,1,1,1,1,2,:)),'b.');
title("model-free");

%%
%log likelihood
LL = zeros(participantNum,alpha1bins,alpha2bins,lumbdabins,omegabins,beta1bins,beta2bins);
   ...
   for a1 = 1:alpha1bins
       for a2 = 1:alpha2bins
           for l = 1:lumbdabins
               for o = 1:omegabins
                   w = omega(o);
                   %combine values (size 2*99)
                   Q1 = w*squeeze(Q_MB_vals(p,a1,a2,l,:,:))+(1-w)* squeeze(Q_MF_vals(p,a1,a2,l,1,1:2,:));
                   for b1 = 1:beta1bins
                       for b2=1:beta2bins
                            temp_LL = 0;
                            for t = 1:sum(Q1(1,:)~=-1)
                        % softmax
                            %stage1
                                temp_prob1 = log(squeeze(exp(beta1(b1) * Q1(:,t))) ./ sum(exp(beta1(b1) * Q1(:,t))));
                                temp_LL = temp_LL + temp_prob1(choice(p,t,1));
                            %stage2
                                temp_prob2 = log(squeeze(exp(beta2(b2) * Q_MF_vals(p,a1,a2,l,2,:,t))) ./ sum(exp(beta2(b2) * Q_MF_vals(p,a1,a2,l,2,:,t))));
                                temp_LL = temp_LL + temp_prob2(choice(p,t,2));
                            end
                            LL(p,a1,a2,l,o,b1,b2) = temp_LL;
                       end
                   end
               end
           end
       end
       disp(["personal progress: "+ a1/10]);
   end
   
   %normalize within-subject so that prob. add up to 1
   LL(p,:) =LL(p,:) - log(sum(exp(LL(p,:)),'all'));
   
end

disp("finished estimating LL");
%% infer params
%(alpha1bins,alpha2bins,lumbdabins,omegabins,beta1binsbeta2bins);
marginal_likelihood_alpha1 = -1*ones(participantNum,alpha1bins);
marginal_likelihood_alpha2 = -1*ones(participantNum,alpha2bins);
marginal_likelihood_lumbda = -1*ones(participantNum,lumbdabins);
marginal_likelihood_omega = -1*ones(participantNum,omegabins);
marginal_likelihood_beta1 = -1*ones(participantNum,beta1bins);
marginal_likelihood_beta2 = -1*ones(participantNum,beta2bins);


for p = 1:participantNum
    marginal_likelihood_alpha1(p,:) = sum(exp(squeeze(LL(p,:,:,:,:,:,:))),[2 3 4 5 6]);
    marginal_likelihood_alpha2(p,:) = sum(exp(squeeze(LL(p,:,:,:,:,:,:))),[1 3 4 5 6]);
    marginal_likelihood_omega(p,:) = sum(exp(squeeze(LL(p,:,:,:,:,:,:))),[1 2 3 5 6]);
    marginal_likelihood_lumbda(p,:) = sum(exp(squeeze(LL(p,:,:,:,:,:,:))),[1 2 4 5 6]);
    marginal_likelihood_beta1(p,:) = sum(exp(squeeze(LL(p,:,:,:,:,:,:))),[1 2 3 4 6]);
    marginal_likelihood_beta2(p,:) = sum(exp(squeeze(LL(p,:,:,:,:,:,:))),[1 2 3 4 5]);
end

%% marginal alpha1 plot
subplot(2,3,1);
for p = 1:participantNum
    plot(logAlpha1,squeeze(marginal_likelihood_alpha1(p,:)),'lineWidth',3); hold on;
end
title("alpha1");

subplot(2,3,2);
for p = 1:participantNum
    plot(logAlpha2,squeeze(marginal_likelihood_alpha2(p,:)),'lineWidth',3); hold on;
end
title("alpha2");

subplot(2,3,3);
for p = 1:participantNum
    plot(logLumbda,squeeze(marginal_likelihood_lumbda(p,:)),'lineWidth',3); hold on;
end
title("lambda");

subplot(2,3,4);
for p = 1:participantNum
    plot(logOmega,squeeze(marginal_likelihood_omega(p,:)),'lineWidth',3); hold on;
end
title("omega");

subplot(2,3,5);
for p = 1:participantNum
    plot(logBeta1,squeeze(marginal_likelihood_beta1(p,:)),'lineWidth',3); hold on;
end
title("beta");

subplot(2,3,6);
for p = 1:participantNum
    plot(logBeta2,squeeze(marginal_likelihood_beta2(p,:)),'lineWidth',3); hold on;
end
title("beta2");

%% calculating expected parameter value for each participant
expected_alpha1 = zeros(participantNum,1);
expected_alpha2 = zeros(participantNum,1);
expected_lumbda = zeros(participantNum,1);
expected_omega = zeros(participantNum,1);
expected_beta1 = zeros(participantNum,1);
expected_beta2 = zeros(participantNum,1);

for p = 1:participantNum
        expected_alpha1(p) = sum(marginal_likelihood_alpha1(p,:).* alpha1);
        expected_alpha2(p) = sum(marginal_likelihood_alpha2(p,:).* alpha2);
        expected_lumbda(p) = sum(marginal_likelihood_lumbda(p,:).* lumbda);
        expected_omega(p) = sum(marginal_likelihood_omega(p,:).* omega);
        expected_beta1(p) = sum(marginal_likelihood_beta1(p,:).* beta1);
        expected_beta2(p) = sum(marginal_likelihood_beta2(p,:).* beta2);
end

params_out = table(expected_alpha1,expected_alpha2,expected_lumbda,expected_omega,expected_beta1,expected_beta2);
writetable(params_out,'/Users/rh/Desktop/first_year/PSYCH251_MF/daw2011/writeup/data/RL_learn_params_out.csv');